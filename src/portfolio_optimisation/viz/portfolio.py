...
class PortfolioVisualiser:
    ...
    Args:
        ...
        all_weights: Map of portfolio name to weights, as a Series or a dict.
    """
    ...
    def plot_efficient_frontier(
        ...
        configure_style()
        fig, axis = _figure(ax)
        risks, returns = efficient_frontier(np.asarray(self.mu), np.asarray(self.s))
        axis.plot(risks, returns, label="Efficient frontier")
    ...
    def plot_dendrogram(
        ...
        **kwargs: Any,
        ...
        if hrp_model.linkage_matrix is None:
            msg = "HRP model must be optimized first."
        ...
        **kwargs: Any,
        ...
        if ordered:
            if not hrp_model.ordered_tickers:
                msg = "HRP model must be optimized to get ordered tickers."
                raise RuntimeError(msg)
