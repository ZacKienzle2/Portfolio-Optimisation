...
class GeneralisedParetoFit:
    ...
    Attributes:
        shape: GPD shape parameter ``xi`` (tail index).
        scale: GPD scale parameter ``beta``.
        threshold: Loss threshold ``u`` above which the tail is modelled.
        exceedance_rate: Fraction of observations above the threshold.
        n_exceedances: Number of exceedances used in the fit.
    """
...
def fit_peaks_over_threshold(
    ...
    *,
    ...
    Args:
        values: Returns or losses.
        threshold_quantile: Quantile of the loss distribution used as the threshold
            ``u`` (for example 0.95).
        kind: ``"return"`` if values are returns, else ``"loss"``.
    ...
    if exceedances.size < 10:
        msg = "Too few exceedances above the threshold to fit a GPD."
...
def evt_value_at_risk(
    ...
    *,
    ...
    Args:
        values: Returns or losses.
        alpha: Tail probability (for example 0.01).
        threshold_quantile: Threshold quantile for the GPD fit.
        kind: ``"return"`` or ``"loss"``.
...
def hill_estimator(
    ...
    *,
    ...
    if not 1 <= k < ordered.size:
        msg = "k must lie in [1, number of positive losses - 1]."
        raise ValueError(msg)
    top = ordered[:k]
