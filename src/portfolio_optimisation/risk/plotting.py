...
def plot_simulation_results(
    ...
    Args:
        simulated_returns: DataFrame with 'simulated_returns'.
        risk_metrics: Dictionary containing 'VaR' and 'CVaR'.
    """
    if simulated_returns.empty or "simulated_returns" not in simulated_returns.columns:
        logger.warning("Empty or invalid DataFrame provided for plotting.")
        return
    ...
    if var_value is None or cvar_value is None:
        logger.warning("Valid VaR or CVaR missing in risk_metrics for plotting.")
        return
