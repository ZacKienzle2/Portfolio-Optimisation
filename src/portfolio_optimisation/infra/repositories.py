...
class YfinanceParquetRepository(MarketDataRepository):
    ...
    Args:
        cache_path: On-disk parquet snapshot, the configured default when None.
        console: Rich console for status output, a new one when None.

    Attributes:
        cache_path: On-disk parquet snapshot. Read when present, written through after a
...
class InMemoryUnitOfWork(UnitOfWork):
    ...
    def __exit__(
        ...
        """Roll back on an exception or when :meth:`commit` was not called."""
