...
def whole_shares(targets: NDArray[np.float64], prices: NDArray[np.float64]) -> NDArray[np.float64]:
    ...
    default absolute gap. Where HiGHS returns no solution, as it did for five
    equal targets at a price of 2.00001 on a budget of 1000, the counts are NaN,
    so a comparison with the rewrite excludes them as it excludes an overspend.
    ...
    return np.full(n, np.nan) if result.x is None else np.rint(result.x[:n])
