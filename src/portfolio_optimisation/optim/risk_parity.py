...
def risk_parity_weights(
    ...
    *,
    ...
    Args:
        ...
        cov_matrix: Covariance to use. Defaults to Ledoit-Wolf shrinkage of ``returns``.
        risk_budgets: Target risk contributions per asset. Defaults to equal budgets
            (``1/N``). Normalised internally.
    ...
    Raises:
        ValueError: If ``risk_budgets`` has a non-positive entry or does not match the
            asset count.
    """
    ...
    if risk_budgets is None:
        ...
        if budgets.shape[0] != n or np.any(budgets <= 0.0):
            msg = "risk_budgets must be positive and match the asset count."
