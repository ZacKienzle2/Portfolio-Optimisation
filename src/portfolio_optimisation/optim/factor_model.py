...
def statistical_factor_covariance(returns: pd.DataFrame, n_factors: int) -> pd.DataFrame:
    ...
    Args:
        ...
        n_factors: Number of principal-component factors to retain.
    ...
    if not 1 <= n_factors <= n_assets:
        msg = "n_factors must lie in [1, n_assets]."
...
def factor_model_covariance(returns: pd.DataFrame, factors: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        returns: Asset returns; columns are tickers.
        factors: Factor returns aligned on the same index; one column per factor (for
            example market, size, value).
    ...
    if len(common_index) == 0:
        msg = "returns and factors share no common observations."
        raise ValueError(msg)
