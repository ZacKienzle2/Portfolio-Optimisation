...
class Settings:
    ...
    Attributes:
        ...
        n_simulations: Monte Carlo simulation paths.
        var_alpha: Tail level for VaR/CVaR.
        var_method: Empirical or parametric VaR.
        linkage_method: Hierarchical-clustering linkage.
        seed: Master seed for every stochastic path. None means non-deterministic.
        std_floor: Lower clamp on standard deviations to avoid division by near-zero
            variance.
        cache_dir: Directory for bootstrap and simulation artefacts.
        data_cache_path: Parquet snapshot for market data.
...
def load_settings(config_path: str | Path | None = None, /, **overrides: Any) -> Settings:
    ...
    Args:
        config_path: Explicit TOML path. Defaults to ``portfolio.toml`` in the working
            directory when present.
        **overrides: Explicit field values; any whose value is not None take top
            precedence.
