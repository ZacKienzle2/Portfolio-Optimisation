# Portfolio_Theory/stochastics.py
import numpy as np
import pandas as pd
...
class SDEFitter:
    ...
    def __init__(self, pricesDf: pd.DataFrame, dt: float = 1 / 252):
        ...
        Args:
            ...
            dt (float, optional): The time step (e.g., 1/252 for daily data).
                                  Defaults to 1/252.
        """
    ...
    def fitGbm(
        ...
        """Fit Geometric Brownian Motion (GBM) to each asset price series.

        The GBM model is commonly used to model asset prices, assuming constant
        drift ($\mu$) and volatility ($\sigma$).

        Args:
            paramBounds (Optional[List[Tuple[float, float]]]): Bounds for the
                MLE minimisation, e.g., for [mu, sigma]. Defaults to [-1.0, 1.0]
                and [1e-5, 5.0].
            initialGuess (Optional[NDArray[np.float64]]): Initial parameter
                guess for [mu, sigma]. Defaults to [0.01, 0.2].

        Returns:
            pd.DataFrame: DataFrame indexed by Ticker with fitted parameters
                          ('mu', 'sigma') and diagnostics (Log-Likelihood, AIC, BIC).

        Raises:
            RuntimeError: If the DataFrame generation fails.
        """
        ...
        for ticker in self.pricesDf.columns:
            ...
            # Suppress verbose output from pymle
            est: Any  # Type from external lib without stubs
    ...
    def fitOu(
        ...
        """Fit Ornstein-Uhlenbeck (OU) process to each asset price series.

        The OU model is used for mean-reverting behaviour, characterised by
        speed of reversion ($\kappa$), mean ($\mu$), and volatility ($\sigma$).

        Args:
            paramBounds (Optional[List[Tuple[float, float]]]): Bounds for the
                MLE minimisation, e.g., for [kappa, mu, sigma].
            initialGuess (Optional[NDArray[np.float64]]): Initial parameter
                guess for [kappa, mu, sigma].

        Returns:
            pd.DataFrame: DataFrame indexed by Ticker with fitted parameters
                          ('kappa', 'mu', 'sigma') and diagnostics.
